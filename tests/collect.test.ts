...
describe("SSH command construction", () => {
  ...
  test("collects docker and podman independently and lists libvirt domains", () => {
    ...
    expect(REMOTE_SCRIPT).toContain("domstats --state --balloon --vcpu");
...
const DOMSTATS_KVM_SAMPLE = `@@HOST@@
lab
2
0 0 0 1/1 1
1 0
...
describe("parseContainers batched virsh domstats", () => {
  const containers = parseContainers(splitSections(DOMSTATS_KVM_SAMPLE));

  test("parses domain stats and deduplicates by domain name", () => {
    expect(containers.map((c) => `${c.runtime}:${c.name}`).sort()).toEqual([
      "kvm:debian",
      "kvm:win10",
    ]);
  ...
  test("maps domstats fields onto container card model", () => {
    const win = containers.find((c) => c.name === "win10");
    expect(win?.runtime).toBe("kvm");
    expect(win?.state).toBe("running");
    expect(win?.status).toBe("running");
    expect(win?.image).toBe("kvm qemu:///system");
    expect(win?.memUsageBytes).toBe(4194304 * 1024);
    expect(win?.memLimitBytes).toBe(8388608 * 1024);
    expect(win?.memPercent).toBe(50);
    const debian = containers.find((c) => c.name === "debian");
    expect(debian?.runtime).toBe("kvm");
    expect(debian?.state).toBe("exited");
    expect(debian?.status).toBe("shut off");
  });
});
