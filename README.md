...
domstats` on `qemu:///system` (falling back to `qemu:///session`), batched
in a single call. Hosts with none of those simply show host metrics — the
containers column appears only when something exists. `inspect` / `domstats`
stay narrow: full inspect JSON and domain XML would leak secrets into the
snapshot.
